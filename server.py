...
def echoServer():
    ...
    while inputs:
        # in case of a change in input socket, output socket don't block and operate
        reading, writing, exceptional = select.select(inputs, outputs, inputs)

        # If server-> new connection, else -> data income
        # If data read is none -> client socket closed connection
        for s1 in reading:

            if s1 is server:
                # ACCEPT NEW CONNECTION
                client_socket, peername = s1.accept()
                port = peername[1]
                print 'Accepted new connection on port', port

                inputs.append(client_socket)
                messages[client_socket] = Queue.Queue() # Init new socket queue
            else:
                # ACCEPT DATA FROM CONNECTION
                data = s1.recv(1024)
                port = s1.getpeername()[1]
                if data:
                    print 'from port', port,'--', data
                    #to_send = 'from port '+ str(port) +': ' + data

                    #Echo back the data
                    messages[s1].put(data)
                    outputs.append(s1)

                else:
                    #THE READ OF SOCKET IN NONE BECAUSE IT HAS BEEN CLOSED
                    print 'connection with', port, 'lost'
                    del messages[s1] # Can't send messages to this socket anymore

                    if s1 in outputs:
                        outputs.remove(s1) # in case a message had to be sent to this client -> remove socket

                    inputs.remove(s1) # remove socket
        ...
        for s2 in writing:
            try:
                if s2 not in outputs: continue
                to_send = messages[s2].get_nowait()
            except Queue.Empty:
                # There are no messages to send client_socket
                outputs.remove(s2)
            else:
                s2.send(to_send)
...
def httpServerExercise():
    base_path = raw_input("Base path for website: ")
    server = socket(AF_INET, SOCK_STREAM)
    server.bind(('0.0.0.0', 80))
    server.listen(5)
    while(1):
        client, peername = server.accept()
        command = client.recv(1000)
        cmd_tokens = command.split(' ')
        if len(cmd_tokens) >= 2 and cmd_tokens[0] == 'GET':
            try:
                file = open(base_path + cmd_tokens[1],'r').read()
